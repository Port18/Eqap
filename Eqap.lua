...
redis1 = require('redis')
faederdx1 = redis1.connect('127.0.0.1', 6379)
...
Status = "انا البوت :) "
elseif redis:sismember(bot_id.."Eqap:Developer:Bot1", user_id) then
Status = redis:get(bot_id.."Eqap:Developer:Bot:Reply"..chat_id) or "Owner 🎖"  
elseif redis:sismember(bot_id.."Eqap:Developer:Bot", user_id) then
Status = redis:get(bot_id.."Eqap:Developer:Bot:Reply"..chat_id) or "Owner"  
elseif redis:sismember(bot_id.."Eqap:President:Group"..chat_id, user_id) then
...
if text and text:match("/start hms(.*)_(%d+)") then 
  ...
  Faeder = faederdx1:get(BOT_ID.."HM:"..faed_dx[1]..faed_dx[2])
  if Faeder then 
...
CmdStart = '\n◦ اهلا بك  \n انا بوت تابع  '..(redis:get(bot_id.."Eqap:Redis:Name:Bot") or "Lisa source")..''..
...
'\n◦ يمكنك كتابة /play او الاوامر الخدميه لرؤيه خدمات الخاص '..
'\n◦ Dev ← ['..UserName_Dev..']'
...
if text and text:match("^رفع OWNER S @(.*)$") and Dev_Bots(msg) then
function FunctionStatus(arg, result)
if (result.id_) then
if (result and result.type_ and result.type_.ID == "ChannelChatInfo") then
send(msg.chat_id_,msg.id_,"عذرا هاذا معرف قناة")   
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^رفع OWNER S @(.*)$")}, FunctionStatus, nil)
end

if text and text:match("^تنزيل OWNER S @(.*)$") and Dev_Bots(msg) then
function FunctionStatus(arg, result)
if (result.id_) then
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^تنزيل OWNER S @(.*)$")}, FunctionStatus, nil)
end
...
keyboard.inline_keyboard = {
{
{text = 'وضع القروبات الخدميه', callback_data="/addsender@"..msg.sender_user_id_},{text = 'وضع القروبات العاديه', callback_data="/addchat@"..msg.sender_user_id_},
},
}
local msg_id = msg.id_/2097152/0.5
https.request("https://api.telegram.org/bot"..token..'/sendMessage?chat_id=' .. msg.chat_id_ .. '&text=' .. URL.escape(Texti).."&reply_to_message_id="..msg_id.."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
